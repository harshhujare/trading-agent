...
STEPS
1. Read CLAUDE.md (rules + journal template).
...
3. Run: python3 scripts/research.py scan. This returns ONE JSON with 27 tickers, each containing: symbol, last_close, ma_20, ma_50, ma_signal (bullish/bearish/mixed), top_headline. Use this as your primary research input — do NOT call bars/news per ticker.
4. Rank the 27 tickers and pick the TOP 5 candidates. Weighting:
   ...
   - bearish or mixed ma_signal → low (consider only if the news is exceptionally strong)
...
6. Create today's journal file using the CLAUDE.md template. Fill in:
   ...
   - ## Market Research — for each TOP 5: 2-3 short bullets (MA read, news, BUY/HOLD/SELL lean). For the other 22: ONE combined line "Other watchlist (no action signal): TICK1, TICK2, ..."
